...
public:
    inline static size_t find_segment_end(const concurrent_vector_base &v) {
        unsigned u = v.my_segment==(&(v.my_storage[0])) ? pointers_per_short_segment
                                                        : pointers_per_long_segment;
        size_t k = 0;
    ...
    inline static void *allocate_segment(concurrent_vector_base &v, size_t k) {
        return v.vector_allocator_ptr(v, k);
    }
    ...
    static void extend_segment_if_necessary(concurrent_vector_base &v, size_t k) {
        ...
        int i = 0; do {
            if(!v.my_storage[i].array) {
                backoff.pause();
                i = 0; continue; // TODO: do we really need counter reset here?
            } else i++;
        } while( i < pointers_per_short_segment);
...
void concurrent_vector_base::internal_reserve( size_type n, size_type /*element_size*/, size_type max_size ) {
    ...
    for( unsigned k = helper::find_segment_end(*this); segment_base(k)<n; ++k ) {
        ...
        my_segment[k].array = helper::allocate_segment(*this, m);
    }
}
...
void concurrent_vector_base::internal_copy( const concurrent_vector_base& src, size_type /*element_size*/, internal_array_op2 copy ) {
    ...
    if( n ) {
        size_type b;
        for( unsigned k=0; (b=segment_base(k))<n; ++k ) {
            helper::extend_segment_if_necessary(*this, k);
            size_t m = segment_size(k);
...
void* concurrent_vector_base::internal_push_back( size_type element_size, size_type& index ) {
    ...
    if( !array ) {
        ...
        if( base==tmp ) {
            ...
            array = helper::allocate_segment(*this, n);
...
concurrent_vector_base::size_type concurrent_vector_base::internal_clear( internal_array_op1 destroy ) {
